...
int						enemy_sees_you(t_enemy *enemy, t_player *player,
							t_map *map);
int						line_of_sight(t_point enemy_pos, t_point player_pos,
							t_map *map);
...
void					create_portal_at_position(t_game *game, int map_x,
							int map_y, int orientation);
...
// shoot/shoot_utils.c
int						is_enemy_in_line_of_fire(t_game *game,
...
void					create_new_portal_at_position(t_game *game, int map_x,
							int map_y, int orientation);
void					reset_and_create_portal_at_position(t_game *game,
							int map_x, int map_y, int orientation);
void					setup_portal_data(t_game *game, int map_x, int map_y,
							int orientation);
void					print_portal_creation(int portal_count, int map_x,
							int map_y, int orientation);
void					print_portal_reset(int map_x, int map_y,
							int orientation);
...
void					render_weapon_pickup(t_game *game,
							t_weapon_pickup *weapon);
void					calculate_weapon_transform(t_game *game,
							t_weapon_pickup *weapon, t_render *render);
void					calculate_weapon_screen_pos(t_game *game,
							t_render *render);
void					draw_weapon_pickup_sprite(t_game *game, t_img *sprite,
							t_point pos, int size);

// core/weapon_loader.c
// int load_weapon_pickup_sprite(t_game *game, t_weapon_pickup *pickup,
//	char *path);
int						count_weapons_in_map(t_game *game);
int						set_weapon_positions(t_game *game);
// int load_weapon_pickup_sprites(t_game *game);
void					disable_weapon_pickup_at_position(t_game *game,
							int map_x, int map_y, int weapon_type);
int						is_not_wall_for_movement(t_map *map, double x,
							double y);
...
void					render_open_door_as_sprite(t_game *game,
							t_open_door *door);
...
void					check_open_doors_on_ray(t_game *game, int column_x,
							double radiant_angle);
void					render_door_on_column(t_game *game, t_open_door *door,
							int column_x, double distance);
int						check_and_render_door_on_column(t_game *game,
							int column_x, t_ray *ray);
void					render_door_column_fixed(t_game *game,
							t_open_door *door, int column_x, double distance);
double					check_open_door_on_ray(t_game *game,
							double radiant_angle);

// Fonctions de rendu
void					render_open_door_ultra_thin(t_game *game, int column_x,
							t_render *renderer, t_ray *ray);
void					render_simple_door_line(t_game *game, int column_x,
							t_render *renderer);
t_open_door				*find_door_at_position(t_game *game, int map_x,
							int map_y);
void					render_open_door(t_game *game, int column_x,
							t_render *renderer, t_ray *ray);
...
void					render_laser_overlay_on_column(t_game *game,
							int column_x);
...
void					use_healgun(t_game *game);
int						load_single_weapon_texture(void *mlx, t_img *tex,
							char *path);
...
void					set_next_frame_timer(t_game *game,
...
int						load_healgun_frame(t_game *game, int frame,
							const char *path);
...
void					setup_portal_one(t_game *game, int map_x, int map_y,
							int orientation);
void					setup_portal_two(t_game *game, int map_x, int map_y,
							int orientation);

// portal/portal_create.c
int						create_portal(t_game *game, int map_x, int map_y,
							int orientation);
int						create_new_portal(t_game *game, int map_x, int map_y,
							int orientation);
int						reset_and_create_portal(t_game *game, int map_x,
							int map_y, int orientation);
void					print_portal_created(int portal_num, int x, int y,
							int orientation);
void					print_portal_reset(int x, int y, int orientation);

// portal/portal_teleport.c
void					teleport_through_portal(t_game *game, int portal_x,
							int portal_y);
t_portal				*get_destination_portal(t_game *game, int portal_x,
							int portal_y);
void					execute_teleportation(t_game *game,
...
int						try_teleport_at_direction(t_game *game, int dest_x,
							int dest_y, int dir);
...
void					update_ray_data(t_game *game, int i,
							double radiant_angle);

// portal/portal_utils2.c
void					init_adjacent_offsets(int adj_offsets[4][2]);
void					check_portal_teleportation(t_game *game);
void					check_nearby_portals(t_game *game, int player_x,
							int player_y);
void					init_check_offsets(int check_offsets[9][2]);
void					check_portal_at_offset(t_game *game, int player_x,
							int player_y, int offset[2]);
double					calculate_distance_to_portal(t_game *game,
							double portal_x, double portal_y);
...
int						load_weapon_pickup_sprite(t_game *game,
							t_weapon_pickup *pickup, char *path);
int						set_weapon_positions(t_game *game);
...
void					calculate_weapon_position(t_game *game,
							t_render *renderer, t_img *weapon);
int						is_healgun_frame_2(t_game *game);
void					update_weapon_animation(t_game *game);
void					draw_weapon_sprite(t_game *game, t_render *renderer,
							t_img *weapon);
void					draw_weapon_row(t_game *game, t_render *renderer,
							t_img *weapon);
void					draw_weapon_pixel(t_game *game,
							t_weapon_pixel_data *data);

// weapon/weapon_specific.c
int						load_hands(t_game *game);
void					setup_hands_texture(t_game *game, int width,
							int height);
int						load_raygun(t_game *game);
int						load_portalgun(t_game *game);
int						load_healgun(t_game *game);
...
void					render_weapon_pickup(t_game *game,
							t_weapon_pickup *weapon);
int						should_skip_weapon_render(t_game *game,
							t_weapon_pickup *weapon);
int						setup_healgun_pickup_sprite(t_game *game,
							t_weapon_pickup *weapon);
void					calculate_weapon_transform(t_game *game,
							t_weapon_pickup *weapon, t_render *render);
void					calculate_weapon_screen_pos(t_game *game,
							t_render *render);
void					setup_weapon_pickup_render(t_game *game,
							t_render *renderer);
void					draw_weapon_pickup_sprite(t_game *game, t_img *sprite,
							t_point pos, int size);
...
void					disable_weapon_pickup_at_position(t_game *game,
							int map_x, int map_y, int weapon_type);
int						is_weapon_at_position(t_weapon_pickup *weapon,
							int map_x, int map_y, int weapon_type);
void					setup_weapon_at_position(t_game *game, int x, int y,
							int *weapon_index);
...
// door/door_core.c
void					open_door(t_game *game);
void					calculate_door_check_position(t_game *game,
							double *check_x, double *check_y);
int						is_valid_door_position(t_game *game, int map_x,
							int map_y);
void					toggle_door_state(t_game *game, int map_x, int map_y,
							char cell);
t_open_door				*find_door_at_position(t_game *game, int map_x,
							int map_y);
...
void					render_open_door(t_game *game, int column_x,
							t_render *renderer, t_ray *ray);
void					calculate_door_texture_coordinates(t_render *renderer,
							t_ray *ray);
...
void					setup_texture_data(t_img *texture, int width,
							int height);
...
// door/door_utils.c
void					render_door_column(t_game *game, int column_x,
							t_render *renderer);
int						calculate_texture_y(t_render *renderer, int cy,
							double h);
void					render_door_pixel(t_game *game, int column_x,
							t_render *renderer, int texture_y);
void					setup_door_at_position(t_game *game, int x, int y,
							int *door_index);
...
// walls/wall_utils.c
int						calculate_texture_y_from_position(int y, int cy,
							double h);
void					set_wall_pixel_color(t_game *game, int x, int y,
							unsigned int color);

void					render_shooted_open_door(t_game *game, int column_x,
							t_render *renderer, t_ray *ray);
void					apply_damage_effect(unsigned int color,
...
void					handle_movement_keys_press(int keycode,
							t_player *player);
void					handle_movement_keys_release(int keycode,
							t_player *player);
void					handle_rotation_keys_press(int keycode,
							t_player *player);
...
int						mouse_move(int x, int y, t_game *game);
void					apply_mouse_rotation(t_game *game, int delta_x,
							int delta_y);
void					clamp_pitch(t_game *game);
void					handle_rotation_keys_release(int keycode,
							t_player *player);
...
void					pickup_portalgun(t_player *player, int map_x,
							int map_y);
void					pickup_healgun(t_player *player, int map_x, int map_y);

// player/pickup/player_pickup_healgun.c
void					pickup_healgun_first_time(t_player *player, int map_x,
							int map_y);
void					pickup_healgun_ammo(t_player *player, int map_x,
							int map_y);
void					reload_healgun_if_empty(t_player *player);
int						is_valid_pickup_position(t_game *game, int map_x,
							int map_y);
...
void					apply_movement_bounds(t_player *player, float *new_x,
							float *new_y);
...
int						is_valid_screen_pixel(t_minimap *mini_map,
							int screen_pixel_x, int screen_pixel_y);
...
void					update_weapon_animation(t_game *game);
void					render_weapon(t_game *game);
void					render_shooted_door_column(t_game *game,
...
void					draw_weapon_pixel_render(t_game *game, t_img *weapon,
							t_render *renderer);
void					draw_weapon_pickup_row(t_game *game,
							t_weapon_pixel_data *data);
void					calculate_pickup_src_coords(t_weapon_pixel_data *data,
							int src_coords[2]);
unsigned int			get_pickup_pixel_color(t_img *sprite,
							int src_coords[2]);
int						is_pickup_pixel_transparent(unsigned int color);
int						is_valid_screen_coord(int coords[2]);
void					set_pickup_pixel_color(t_game *game, int coords[2],
							unsigned int color);
void					setup_weapon_pickup_render(t_game *game,
							t_render *renderer);
...
void					setup_enemy_render_params(t_game *game,
							t_render *render);
int						calculate_sprite_bounds(t_render *render, int *start,
							int *end);
...
void					draw_sprite_pixel_at_pos(t_game *game,
							t_weapon_pixel_data *data);

// enemy/enemy_sprite_utils.c
t_img					*get_death_sprite(t_enemy *enemy);
int						setup_death_render(t_game *game, t_enemy *enemy,
							t_render *renderer);
void					apply_death_modifiers(t_enemy *enemy,
							t_render *renderer);
...
double					calculate_distance_to_weapon(t_game *game,
							t_weapon_pickup *weapon);

int						process_line(char *line, t_scene_data *scene,
							t_game *game);
int						parse_texture(char *path_str, char **texture_path);
int						parse_color(char *color_str, int *color);
int						add_map_line(char *line, t_game *game,
							t_scene_data *scene);
int						validate_and_load_scene(t_scene_data *scene,
							t_game *game);
