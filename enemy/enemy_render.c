...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/27 19:54:56 by tcaccava          #+#    #+#             */
/*   Updated: 2025/05/27 19:56:50 by tcaccava         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_render *render)
{
	...
	render->floor_x = inv_det * (game->player.dir_y * dx - game->player.dir_x
			* dy);
...
void	calculate_enemy_screen_pos(t_game *game, t_render *render)
{
	...
	render->x = (int)((DISPLAY_WIDTH / 2) * (1 + render->floor_x
				/ render->floor_y));
	distance_to_projection_plane = (DISPLAY_WIDTH / 2.0) / tan(game->player.fov
			/ 2.0);
...
int	check_enemy_occlusion(t_game *game, t_render *render)
{
	...
	sprite_start_pixel = render->x - render->sprite_size / 2;
	if (sprite_start_pixel < 0)
		sprite_start_pixel = 0;
	sprite_end_pixel = sprite_start_pixel + render->sprite_size - 1;
	...
	while (++i < 5)
	{
		col = sprite_start_pixel + i * (sprite_end_pixel - sprite_start_pixel)
			/ 4;
...
void	setup_enemy_render_params(t_game *game, t_render *render)
{
	...
	render->draw_end = (DISPLAY_HEIGHT - render->sprite_size) / 2 + game->pitch
		+ y_offset;
}
